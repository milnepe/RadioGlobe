...
class Positional_Encoders:
    ...
    async def fetch(self):
        """Get the current latitude and longitude readings."""
        ...
        if readings:
            ...
            if self.latch_stickiness is None:
                ...
                if lon_difference > self.latch_stickiness:
                    self.latch_stickiness = 0
        await asyncio.sleep(0.5)  # Simulate some delay for reading
        return readings[0], readings[1] if readings else (None, None)
...
async def run():
    ...
    encoders = Positional_Encoders(0, 0)

    task = asyncio.gather(encoders.fetch())
    try:
        # get the return value from the wrapped coroutine
        coords_lat, coords_long = task.result()
    ...
    encoders.zero()  # Zero the encoders to set offsets
    print(
        f"Encoder offsets set to: {encoders.latitude_offset}, {encoders.longitude_offset}"
    )

    await asyncio.sleep(1)  # Allow some time for initial readings

    while True:
        coords_lat, coords_long = await encoders.fetch()
        print(f"Readings: Latitude {encoders.latitude}, Longitude {encoders.longitude}")
