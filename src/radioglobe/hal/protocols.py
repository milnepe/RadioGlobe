...
class HardwareComponent(Protocol):
    ...
    Deliberately does NOT declare start() - five of six concrete start()
    methods are synchronous, but AsyncButtonManager.start() is `async def`
    (buttons.py:110). Forcing one signature would misdescribe a real class,
    so each role protocol below declares its own start() instead.
    """
