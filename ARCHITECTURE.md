...
**Residual import gap:** `main.py` imports `create_button_manager`/`ButtonCallbacks` from `radioglobe.hal.buttons` at module scope, and `hal/buttons.py` imports `evdev` at module scope. So `import radioglobe.main` (and therefore `radioglobe.cli`) requires `evdev` to be importable (real or stubbed) — this doesn't go through `build_hardware()`'s deferred-import guarantee at all. Any test importing `radioglobe.main` needs the same `sys.modules.setdefault("evdev", MagicMock())` stub `tests/buttons_test.py` uses — see `tests/main_test.py`. Bare `import radioglobe.hal` still has no such requirement, since `hal/__init__.py` doesn't import `.buttons` either.
...
All follow the same style: plain `unittest.TestCase`/`IsolatedAsyncioTestCase`, in-memory fixture data, no mocking framework. `buttons_test.py` and `main_test.py` both stub `evdev` in `sys.modules` before importing `radioglobe.hal.buttons`/`radioglobe.main`, since `hal/buttons.py` imports evdev at module scope (§4.7) and `main.py` transitively imports it (§4.14's residual import gap). `hal/fake_test.py` and `hal/protocols_test.py` need no such stub — neither imports `radioglobe.hal.buttons`, `radioglobe.hal.rgb_led`, or `radioglobe.main`, and bare `import radioglobe.hal` doesn't either. None of the unit tests need the `pi` extra installed (§8); only `tests/integration/` does.
