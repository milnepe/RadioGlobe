...
│       ├── hal/                      # Hardware Abstraction Layer: Protocols + fakes + factory (see §4.14)
...
(`App.__init__` used to build all 6 hardware wrappers directly as a flat
...
That specific tradeoff was revisited later, once testability off-Pi became a
goal in its own right — see §4.14: `App.__init__` now takes 5 of its 6
hardware objects as constructor parameters, and a `hal/` package provides
Protocol interfaces, fakes, and a `build_hardware()` factory. `AsyncButtonManager`
(the 6th) still isn't constructor-injected, for the same category of reason
button dispatch stayed on `App` here — see §4.14 for why.
...
Reads a quadrature rotary encoder wired to GPIO pins 18 and 17 — these are the encoder's
two switch/channel outputs (A/B), not a clock/direction pair — but unlike every other
GPIO device in this project, decoding happens in the **kernel**, not in Python.
`install.sh` adds `dtoverlay=rotary-encoder,pin_a=18,pin_b=17,relative_axis=1` to
`/boot/firmware/config.txt`, which binds the stock `drivers/input/misc/rotary_encoder.c`
driver to those two pins and does the gray-code decode into `EV_REL`/`REL_X` events.
`pin_a`/`pin_b` were swapped from an earlier `pin_a=17,pin_b=18` to correct the physical
CW/CCW direction — confirmed working on-device.
...
- `_on_readable()` pushes `_POLARITY * sign(event.value)` onto `self.queue` (an
  `asyncio.Queue[int]`, via `put_nowait`) directly for each `REL_X` event — no
  accumulation or timer. An earlier version added a software debounce/coalescing layer
  here to filter contact-bounce bursts (see `docs/KERNEL_ROTARY_ENCODER_INVESTIGATION.md`
  §9); on-device testing after the pin swap showed the raw per-event approach reliable, so
  it was removed. A single `_POLARITY` constant corrects for physical wiring, same role as
...
(the colour-name → RGB-tuple mapping) is built from them rather than
separately hardcoding the same 5 string keys — previously these were two
independent definitions of the same vocabulary with no structural link, so a
typo in either place would have broken silently. `main.py` and the
integration test scripts (`led_test.py`, `main_test.py`,
`streaming_cvlc_test.py`) import the colour names from `radioglobe.rgb_led`
now, not `radioglobe.constants`.
...
Added to make previously hardware-locked `App` logic (the dial/encoder event
loops, LED-flash-on-event behavior, display/audio call sequencing) unit-testable
off a Pi, and to let the base package install and import cleanly on any
platform. Three files, all new, none of which required changing `dial.py`,
`positional_encoders.py`, `buttons.py`, `rgb_led.py`, `display.py`, or
`audio_async.py`:

- **`protocols.py`** — one `typing.Protocol` per hardware role (`DialProtocol`,
  `PositionalEncodersProtocol`, `ButtonManagerProtocol`, `RGBLedProtocol`,
  `DisplayProtocol`, `AudioPlayerProtocol`), matching each real class's public
  method signatures exactly. `Protocol` uses structural typing, so the six
  existing classes already satisfy these interfaces by shape alone — no
  inheritance, no changes to those modules. A shared `HardwareComponent` base
  declares only `async def stop(self) -> None`, deliberately not `start()`:
  five of the six real `start()` methods are synchronous but
  `AsyncButtonManager.start()` (`buttons.py:110`) is `async def`, and forcing
  one signature across all six would misdescribe a real class.
- **`fake.py`** — `FakeDial`, `FakePositionalEncoders`, `FakeButtonManager`,
  `FakeRGBLed`, `FakeDisplay`, `FakeAudioPlayer`. Each satisfies its Protocol
  and exposes simple test hooks (`push_turn()`, `set_position()`,
...
itself; `nav` stays optional since `Navigator` has no hardware dependency.
`cli.py` and `main.py`'s `__main__` block are the only two real call sites,
both now `App(*build_hardware()).run()`.

**`AsyncButtonManager` is not constructor-injected.** It's built inside
`run()` (`main.py`) because it needs app-bound callback methods
...
`dependencies` into an optional `pi` extra (`pip install .[pi]`) as part of
this change — see §8.
...
**HAL note:** `hal/protocols.py` (§4.14) describes each role's real notification
shape as-is — `DialProtocol.queue`, `PositionalEncodersProtocol.updated`,
`ButtonManagerProtocol.event_queue`, `DisplayProtocol.changed` — rather than
forcing a single uniform callback/event interface. A HAL that hid the
...
**Dial switch pins (removed):** `PIN_DIAL_CLOCK`/`PIN_DIAL_DIR` (BCM 17/18 — the names
predate the kernel-driver migration and don't reflect the hardware: both pins read one
of the encoder's two quadrature switch outputs, neither is a clock or a direction line)
used to exist in `radio_config.py` despite having zero Python consumers — the kernel
`rotary_encoder` driver reads the pins directly from `install.sh`'s
`dtoverlay=rotary-encoder,pin_a=18,pin_b=17,...` line (§4.6), which is the only place
they're configured. They were deleted rather than relocated; `install.sh` now has a
comment marking that line as the single source of truth for those two pins.
